...
public class KYH_Mover : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        Vector3 rayStart = col.bounds.center;
        Physics.Raycast(rayStart, Vector3.down, out RaycastHit _hit, 2f, LayerMask.GetMask("ground"),
         QueryTriggerInteraction.Ignore);
        rideHeight = _hit.distance;
        //stepLength = rideHeight - col.height*0.5f;
        // InitCapsuleCollider();
    }
    ...
    public void CheckGround()
    {
        ...
        {
            Vector3 rayStart = col.bounds.center ;
            isGrounded = Physics.Raycast(rayStart, Vector3.down , out RaycastHit _hit, rideHeight+stepLength, LayerMask.GetMask("ground"), QueryTriggerInteraction.Ignore);

            Debug.DrawRay(rayStart, Vector3.down * (rideHeight + stepLength), Color.red);

            if (isGrounded == false) { return; }

            float _distanceToGo = rideHeight - _hit.distance;
            //  stepLength - _hit.distance ;
            Debug.Log($"Length : {_distanceToGo},  staryPos : {rayStart}");
